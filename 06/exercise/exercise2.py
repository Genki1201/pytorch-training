...
if __name__ == "__main__":
    ...
    data_directory_path = Path(data_directory).resolve() #絶対パスの取得
    folder_list = list(data_directory_path.glob("*")) #配下にある0から9の写真入りフォルダを取得
    file_list = [] #すべての写真を各フォルダから取得
    for folder in folder_list:
        file_path_list = list(folder.glob("*")) #1つのフォルダ内のすべての写真ファイルをリストで取得
        file_list += file_path_list

    data_size = len(file_list)

    # データセットを訓練データとテストデータをデータセットを作る前に分割
    train_ratio = 0.8
    ...
    #mydatasetのインスタンス化（Image_transformのインスタンス化も同時に行っている）
